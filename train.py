...
def auto_eval(config, vocab, model_F, test_iters, global_step, temperature):
    ...
    evaluator = Evaluator()
    ref_text = evaluator.hate_ref

    
    acc_neg = evaluator.hate_acc_0(rev_output[0])
    acc_pos = evaluator.hate_acc_1(rev_output[1])
    bleu_neg = evaluator.hate_ref_bleu_0(rev_output[0])
    bleu_pos = evaluator.hate_ref_bleu_1(rev_output[1])
    ppl_neg = evaluator.bert_ppl(rev_output[0])
    ppl_pos = evaluator.bert_ppl(rev_output[1])
    ...
    print(('[auto_eval] acc_pos: {:.4f} acc_neg: {:.4f} ' + \
          'bleu_pos: {:.4f} bleu_neg: {:.4f} ').format(acc_pos, acc_neg, bleu_pos, bleu_neg)  + \
    ...
    with open(eval_log_file, 'a') as fl:
        print(('iter{:5d}:  acc_pos: {:.4f} acc_neg: {:.4f} ' + \
                'bleu_pos: {:.4f} bleu_neg: {:.4f} ').format(global_step, acc_pos, acc_neg, bleu_pos, bleu_neg) + \
                'ppl_pos: {:.4f} ppl_neg: {:.4f}\n'.format(global_step, acc_pos, acc_neg, bleu_pos, bleu_neg, ppl_pos, ppl_neg,), file=fl)

    with open(save_file, 'w') as fw:
        print(('[auto_eval] acc_pos: {:.4f} acc_neg: {:.4f} ' + \
               'bleu_pos: {:.4f} bleu_neg: {:.4f}\n ').format(acc_pos, acc_neg, bleu_pos, bleu_neg) + \
               'ppl_pos: {:.4f} ppl_neg: {:.4f}\n'.format(
           acc_pos, acc_neg, bleu_pos, bleu_neg, ppl_pos, ppl_neg,
       ), file=fw)
